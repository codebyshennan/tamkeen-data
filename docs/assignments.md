...
Underscore-prefixed directories (`assignments`) are excluded by Jekyll's default rules; that is why the assignment files do not appear under `_site/`. Two options:

1. **Rename** every `assignments/` directory to `assignments/` (≈10 dirs, ~80 references in markdown/HTML). Cleanest, but a large mechanical change.
2. **Register as a collection** in `_config.yml`:
   ...
   then move files into a top-level `assignments/` collection. Less invasive but changes URLs.
