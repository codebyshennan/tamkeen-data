...
A simple division field; Looker Studio evaluates this per-row before aggregation.

Sales Growth

Divides this month's total by last month's using subqueries with `DATE_SUB`, then subtracts 1 to get a percentage change.

Customer Segmentation

A `CASE` expression buckets customers into High, Medium, or Low value tiers based on their aggregated sales.

2. Advanced Functions:

Moving Average

`ROWS BETWEEN 2 PRECEDING AND CURRENT ROW` creates a 3-period window that slides forward with each date, smoothing volatility.

Running Total

`ROWS UNBOUNDED PRECEDING` starts from the first row of the partition, accumulating sales into a running sum.

Percent of Total

Divides each row's sales by the grand total from a subquery, giving each row its share of overall revenue.
...
`@threshold` references a Looker Studio parameter control; the `CASE` maps the viewer's selection to a numeric value.

Dynamic Date Filter

`@date_range` lets viewers pick a window; `DATE_SUB` computes the cutoff date dynamically, returning a boolean filter condition.
