...
* [3.1 Intro to data visualization](../3.1-intro-data-viz/) and basic comfort with spreadsheets or SQL-style fields (dimensions vs measures).
...
`FORECAST_INDICATOR` with `'multiplicative'` seasonality and 95% confidence interval forecasts 6 periods ahead from the aggregated sales measure.

K-Means Clustering

`KMEANS` partitions records into 3 clusters using Euclidean distance on two dimensions, requires Tableau's Analytics model.

Statistical Test

`T_TEST` runs a two-tailed t-test between two groups at the 95% confidence level to test for significant differences in means.
