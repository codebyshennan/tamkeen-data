# Module 1: Data fundamentals

**After this module:** You build a practical base for the rest of the course: what data work looks like in organizations, how to read and write Python for data tasks, how to think statistically, how to compute with arrays in NumPy, and how to work with tables in pandas.

## Overview

**Who this is for:** Learners who have completed [Module 0: Prep](../0-prep/) (or equivalent: editor, notebooks, and a working Python environment). No prior data science career experience is assumed.

This module moves from concepts (analytics, ethics, collection) into Python, then statistics, then NumPy and pandas, the stack you will use in later modules for wrangling, visualization, and modeling.

**How to study it:** Work submodule order **1.1 → 1.5**. Read the narrative first, then run code in order; skip ahead only when you are reviewing. If a page feels like a list of facts, slow down and run one example end-to-end, understanding beats coverage.

### Video

_IBM Technology, What is data science?_

## Prerequisites

* Comfortable using a computer, files, and a browser
* Module 0 complete, or you can run Python in VS Code, Jupyter, or Colab
* Willingness to run short code examples and read errors (especially in sections 1.2 onward)

> **Note:** Lesson pages are built with [GitHub Flavored Markdown (GFM)](https://github.github.com/gfm/) on Jekyll. **Contributors:** authoring standards are in `docs/meta/DOCUMENTATION_GUIDELINES.md` in the repository (the `meta/` folder is excluded from the public Jekyll build).

**Code blocks:** When adding lesson snippets, follow the title / Purpose / Walkthrough pattern in [CODE-BLOCK-PATTERN.md](CODE-BLOCK-PATTERN.md).

## Submodule path

Work through these in order; each README lists its own lessons and time estimate.

| Order | Submodule                                                                      | Focus                                                               |
| ----- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------- |
| 1.1   | [Introduction to data analytics](1.1-intro-data-analytics/)                    | Analytics vs data science, lifecycle, collection, privacy, security |
| 1.2   | [Introduction to Python](1.2-intro-python/)                                    | Syntax, structures, control flow, functions, OOP, modules           |
| 1.3   | [Introduction to statistics](1.3-intro-statistics/)                            | Probability, distributions, one- and two-variable summaries         |
| 1.4   | [Data foundation: linear algebra & NumPy](1.4-data-foundation-linear-algebra/) | Arrays, vectorized math, core linear algebra with NumPy             |
| 1.5   | [Data analysis with pandas](1.5-data-analysis-pandas/)                         | Series, DataFrame, alignment, indexing, grouping patterns           |

## Module-level next steps

After you finish Module 1, continue to Module 2: start with [SQL fundamentals](../2-data-wrangling/2.1-sql/), or use the [full curriculum](../curriculum.md) for the full path.
