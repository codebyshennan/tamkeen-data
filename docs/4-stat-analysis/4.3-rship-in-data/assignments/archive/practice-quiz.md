...
3. **reliable standard errors**: keep OLS estimates but use HC standard errors (e.g., `statsmodels` `cov_type='HC3'`)

```python
import statsmodels.api as sm
import numpy as np

# Using reliable standard errors
X = sm.add_constant(np.random.normal(0, 1, (200, 2)))
y = 2 + X[:, 1] * 3 + np.abs(X[:, 1]) * np.random.normal(0, 1, 200)  # heteroscedastic

model = sm.OLS(y, X).fit(cov_type='HC3')  # reliable SEs
...
------------------------------------------------------------------------------
const          1.9491      0.074     26.368      0.000       1.804       2.094
x1             3.1220      0.112     27.831      0.000       2.902       3.342
x2            -0.0604      0.071     -0.845      0.398      -0.200       0.080
==============================================================================
```
</details>
