...
objectives:
  ...
  - >-
    Outline the seven-step machine-learning lifecycle from problem definition to
    deployment.
  - >-
    Recognise the bias-variance tradeoff and read learning curves to diagnose
    under- and overfitting at a glance.
  - >-
    Run a four-line sklearn `fit`/`predict` loop on toy data to anchor the
    supervised-learning idea in code.
---
...
Four (size, price) pairs serve as toy training data; `sizes` is a list of lists because sklearn expects a 2D feature matrix even for a single feature.

Fit and Predict

`model.fit` learns the slope and intercept; `predict` extrapolates to 1750 sq ft, this is the complete supervised learning loop in four lines.

Visualize the Fit

Plotting the data, the fitted line, and the prediction makes the linear relationship concrete, the star lands on the line at 1750 sq ft, exactly where the model interpolates.

<figure><img src="../../../.gitbook/assets/what-is-ml_fig_1.png" alt="what-is-ml"><figcaption><p>Figure 1: Linear regression on toy house data</p></figcaption></figure>
