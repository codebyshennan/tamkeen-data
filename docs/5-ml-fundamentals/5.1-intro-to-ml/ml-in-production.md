...
objectives:
  - >-
    Explain why a trained model is only a small part of a production ML system,
    and name the major components around it.
  - >-
    Distinguish the offline (training) world from the online (serving) world,
    and say why features must stay consistent across both.
  - >-
    Choose a serving pattern, batch, online, streaming, or edge, based on when
    inputs are known and how fast a prediction is needed.
  - >-
    Map the bias-variance ideas you already know (train/test split, leakage,
    overfitting, the iteration loop) onto their production counterparts.
---
...
The entire architecture exists to connect these two worlds _correctly_. Everything below is a piece of that connection.
...
| **Online / real-time** | Inputs are only known at request time                | A user types a _new_ address → predict on the spot via an API (under \~100 ms)     |
...
![Amazon SageMaker Model Monitor workflow: a SageMaker training job produces a model served by a SageMaker endpoint to applications; a baselining job creates baseline statistics and constraints; data-drift and model-quality monitoring jobs compare captured requests and predictions against the baseline and inference ground truth, emit results to Amazon CloudWatch Metrics, and drive training-data updates, retraining, and model updates](../../../.gitbook/assets/sagemaker-model-monitor.jpg)

_Source:_ [_Monitoring in production ML models at large scale using Amazon SageMaker Model Monitor_](https://aws.amazon.com/blogs/machine-learning/monitoring-in-production-ml-models-at-large-scale-using-amazon-sagemaker-model-monitor/) _(AWS Machine Learning Blog)._

### The loop never stops

True outcomes flow back, become new training data, and trigger retraining. This is the **iteration loop from the practitioner's view, automated and never-ending.** In software, automating build-and-release is _CI/CD_; in ML, teams add a third **C**, **CT (continuous training)**, because the model must keep relearning as the world moves. The discipline of doing this well is called **MLOps**.
...
| Data leakage                       | Skew is just leakage you discover _after_ you ship                                              |
...
> Nothing you learned this module gets thrown away in production. It gets _automated and put on a clock._
...
* **Training-serving skew from duplicated feature code.** If the feature logic in your training notebook and your serving app are written twice, they _will_ drift apart. Compute features through one shared path (this is the whole point of a feature store).
...
* **Confusing batch and online because the model is the same.** The trained model can be identical; the _architecture_ around it (precompute-and-store vs. predict-per-request) is a deliberate, separate decision.
...
* [**Module 5, Deploying Machine Learning Models**](https://github.com/DataTalksClub/machine-learning-zoomcamp/tree/master/05-deployment): saving/loading a model with `pickle`, wrapping it in a web service (Flask), managing dependencies (Pipenv), containerising with Docker, and deploying to the cloud. This is the practical version of the _registry → serving_ path above.
