...
objectives:
  - >-
    Distinguish **parameters** (learned from data) from **hyperparameters** (set
    before training) and list the load-bearing ones for common algorithms.
  - >-
    Run `GridSearchCV`, `RandomizedSearchCV`, and Bayesian optimisation (e.g.,
    scikit-optimize, Optuna), and pick between them based on search-space size
    and compute budget.
  - >-
    Always wrap preprocessing + model in a `Pipeline` so the search refits
    transformers per fold without leaking validation data; use `step__param`
    keys.
  - >-
    Avoid the common traps: tuning on the test set, optimising the wrong scoring
    metric, and reporting tuned CV scores as final test scores instead of
    held-out.
---
...
![Grid Search Results](../../../.gitbook/assets/grid_search_results.png)
...
Generate a 1000-sample classification problem and hold 20% back as the final test set, this test set is touched only once after the search finishes.

Parameter Grid

Four hyperparameters with 3-4 values each, 108 total combinations that `GridSearchCV` will evaluate exhaustively.

GridSearchCV Setup

`cv=5` means 5-fold cross-validation per candidate; `n_jobs=-1` parallelizes across all CPU cores; `verbose=1` prints per-fold progress.

Fit, Report, and Test

After searching, `best_estimator_` is refit on the full train set with the winning params; evaluate it once on the held-out test set for an unbiased accuracy estimate.
...
![Random Search Results](../../../.gitbook/assets/random_search_results.png)
...
Instead of discrete grids, use scipy distributions, `randint` for integers and `uniform` for floats, to sample a richer parameter space with the same budget.

Randomized Search Setup

`n_iter=50` draws 50 random combinations; this covers far more of the space than a fixed grid while running in a fraction of the time.

Fit and Report

Same reporting pattern as grid search: print best params and CV score, then evaluate the refitted best estimator on the held-out test set.
...
![Bayesian Optimization](../../../.gitbook/assets/bayesian_optimization.png)
...
Define typed integer and continuous bounds for each hyperparameter that the Bayesian surrogate model will explore.

BayesSearchCV Setup

Configure 50 surrogate-guided trials with 5-fold CV, optimizing accuracy using all available CPU cores.

Fit and Evaluate

Run the Bayesian search, then print the best parameter set, CV score, and final test accuracy.
...
Generate a classification dataset and define a small hyperparameter grid for the nested search.

Nested CV Loop

For each outer fold, run an independent inner GridSearchCV to select hyperparameters, then score the winner on the held-out outer test fold.

Unbiased Estimate

Report mean ± std across outer folds as an unbiased generalization estimate uncorrupted by hyperparameter search.
...
Generate and split a classification dataset so the objective function can evaluate on training data only.

Objective Function

Define the Optuna objective: sample integer hyperparameters via `trial.suggest_int`, then return mean 5-fold CV accuracy.

Study and Results

Create a maximization study, run 100 trials with automatic pruning, and print the best parameters and score found.
