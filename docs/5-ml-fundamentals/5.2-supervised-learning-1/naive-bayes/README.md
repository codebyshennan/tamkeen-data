# Naive Bayes

**After these lessons, you can apply Gaussian, Multinomial, and Bernoulli Naive Bayes classifiers, understand the conditional independence assumption, and use them for text classification.**

## Overview

Naive Bayes applies Bayes' theorem with a strong (naive) independence assumption between features, fast, scalable, and surprisingly effective for text. Prerequisites: [5.1 Intro to ML](../../5.1-intro-to-ml/README.md).

## Lesson Path

1. [Introduction](1-introduction.md)
2. [Math foundation](2-math-foundation.md)
3. [Types of Naive Bayes](3-types.md)
4. [Implementation](4-implementation.md)
5. [Advanced topics](5-advanced-topics.md)
