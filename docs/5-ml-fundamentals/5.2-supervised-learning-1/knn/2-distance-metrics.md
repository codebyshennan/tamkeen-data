...
objectives:
  ...
  - >-
    Pick a metric that fits the data: Euclidean for raw geometry, Manhattan for
    grid-like features, cosine for direction (text and embeddings).
  - >-
    Always scale features before kNN with `StandardScaler` or `MinMaxScaler` so
    large-range columns do not dominate distance.
---

# Understanding Distance Metrics in KNN

**After this lesson:** you can explain Understanding Distance Metrics in KNN and try the examples in your own notebook.

## Overview

Compares **distance metrics** (Euclidean, Manhattan, Minkowski, etc.) and why scaling features first is non-negotiable for kNN.

[Introduction](1-introduction.md) sets the voting story; [5.2 README](../) places kNN among other learners.
...
_Always apply `StandardScaler` or `MinMaxScaler` before using any distance-based method, a salary column in dollars will dwarf an age column in years._
...
Suppose you're predicting house prices using these features:
...
`StandardScaler` normalizes each feature to zero mean and unit variance, critical for distance-based models so features with larger ranges don't dominate the distance calculation.

Compare Metrics

Loop over Euclidean and Manhattan metrics, fit a `KNeighborsRegressor` with each, and collect R² scores, comparing them reveals which metric better reflects similarity in this feature space.
