...
objectives:
  - >-
    Explain SVM as a maximum-margin classifier: fit the boundary that maximises
    distance to the nearest training points.
  - >-
    Identify support vectors, the hyperplane, and the margin in a 2D scatter,
    and explain why only the support vectors define the boundary.
  - >-
    Recognise where SVMs win (small-to-medium tabular data, high-dimensional
    text) and where they struggle (very large `n`, calibrated probabilities).
---

# Introduction to Support Vector Machines (SVM)

**After this lesson:** you can explain Introduction to Support Vector Machines (SVM) and try the examples in your own notebook.

## Overview

**SVMs** fit a **maximum-margin** boundary between classes (or regress with an \\(\varepsilon\\)-insensitive tube); **kernels** lift features so nonlinear boundaries are possible. **Prerequisites:** [5.2 README](../); comfort with vectors from Module 1 helps for [2-math-kernels.md](2-math-kernels.md).
...
![SVM Decision Boundary](../../../../.gitbook/assets/svm_decision_boundary.png)

_Figure: SVM decision boundary (solid line) with margins (dashed lines) and support vectors (points with black edges)_
...
3. **Probabilistic Predictions**
   ...
   * When uncertainty estimates are important
...
3. **Ignoring Class Imbalance**
   * Use class\_weight parameter for imbalanced data
