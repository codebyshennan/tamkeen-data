# Module 5 Assessment

> **Submit your work on Skills Union →** [https://skillsu.com/member/assessment](https://skillsu.com/member/assessment)

**Mandatory** • 6 questions • Covers Machine Learning fundamentals.

Try each question closed-book first. Click **Show hint** if you get stuck, hints point you at the relevant lesson section and how to think about it, without naming the answer.

***

**Q1.** What is the key difference between Traditional Programming and Machine Learning?

* Traditional programming is only used for web development
* Machine learning doesn't require any data
* Traditional programming is generally faster than machine learning
* Traditional programming involves explicitly programming rules and logic, while machine learning involves training a model using data

<details>

<summary>Show hint</summary>

* **Where:** [Intro to ML](../5.1-intro-ml/), what ML is.
* **Think:** Three options make absolute claims that are false on their face ("only web", "no data", "always faster"). The remaining option draws the textbook contrast, **rules written by humans** vs **patterns learned from data**.

</details>

**Q2.** Which is _not_ a type of Machine Learning?

* Stochastic Learning
* Reinforcement Learning
* Unsupervised Learning
* Supervised Learning

<details>

<summary>Show hint</summary>

* **Where:** [Intro to ML](../5.1-intro-ml/), ML categories.
* **Think:** The three classic ML categories all describe what kind of **feedback signal** the model gets (labels, no labels, rewards). One option uses a word that describes **randomness**, not a learning paradigm.

</details>

**Q3.** Which part of the machine learning process deals with choosing the appropriate algorithm?

* Data preprocessing
* Model evaluation
* Model selection
* Hyperparameter tuning

<details>

<summary>Show hint</summary>

* **Where:** [ML workflow](../5.1-intro-ml/), the ML pipeline.
* **Think:** Read each option literally. Preprocessing cleans data, evaluation measures performance, tuning adjusts knobs. The one whose name **literally** describes "picking which model/algorithm" wins.

</details>

**Q4.** Which is _not_ part of features used by feature engineering?

* Text Features
* Categorical Features
* Location Features
* Temporal Features

<details>

<summary>Show hint</summary>

* **Where:** [Feature engineering](../5.1-intro-ml/).
* **Think:** Standard feature engineering taxonomies group features by **data type**, text, categories, time. One option names a **domain** (geography), which is usually treated as a special case of one of the other types rather than its own category.

</details>

**Q5.** Which is _not_ an algorithm used for supervised learning?

* K-means clustering
* Support vector machine
* Decision Trees
* Naive Bayes

<details>

<summary>Show hint</summary>

* **Where:** [Supervised vs unsupervised](../5.1-intro-ml/).
* **Think:** Supervised learning needs **labels**. _Clustering_ groups data **without** labels. One option's name literally contains the word "clustering", that's the giveaway.

</details>

**Q6.** Which algorithm looks at the "k" closest examples in your data and makes predictions based on their characteristics?

* Linear Regression
* Decision Tree
* Support Vector Machine
* K-Nearest Neighbors (KNN)

<details>

<summary>Show hint</summary>

* **Where:** [Classification algorithms](../5.1-intro-ml/), KNN.
* **Think:** The question literally says "the k closest examples". One option's name contains both the letter **k** and the words **nearest neighbors**.

</details>
